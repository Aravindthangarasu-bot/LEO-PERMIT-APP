...
export default function ApplicationNotificationToast() {
  ...
  const openApplication = () => {
    ...
    markNotificationRead(notification.id);
    setNotificationId(null);
    navigate(getApplicationNotificationPath(user.role, notification.applicationId));
  };
  ...
  return (
    <aside role="status" aria-live="polite" style={{ position: 'fixed', right: 24, bottom: 24, width: 'min(380px, calc(100vw - 32px))', background: statusConfig.bg, border: `1px solid ${statusConfig.color}55`, borderLeft: `4px solid ${statusConfig.color}`, borderRadius: 8, boxShadow: '0 18px 45px rgba(15, 23, 42, 0.2)', padding: 16, zIndex: 2000 }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        <button type="button" onClick={openApplication} style={{ flex: 1, minWidth: 0, padding: 0, border: 0, background: 'transparent', textAlign: 'left', cursor: 'pointer', color: 'inherit' }} aria-label={`Open application ${notification.applicationId}`}>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 8, fontSize: 11, fontWeight: 700, color: statusConfig.color }}>{notification.applicationId} · {statusConfig.label} <ExternalLink size={12} /></span>
        </button>
